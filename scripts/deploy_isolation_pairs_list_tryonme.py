"""Deploy isolation duplicate rejection + admin pairs list to tryonme."""
...
def main() -> int:
    ...
    status, pairs = _json_request("GET", "/isolation/pairs?limit=5", token=token)
    if status != 200 or "pairs" not in pairs or "counts" not in pairs:
        raise RuntimeError(f"pairs list failed: {status} {pairs}")
    print("pairs_list_ok", json.dumps({"total": pairs.get("total"), "counts": pairs.get("counts")}))
    return 0
